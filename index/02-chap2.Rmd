...
Missing data is common in the social science research due to survey item-nonresponse or data recording errors, and it is no exception for the OPM dataset. Based on the mechanisms of missing data, there are three types of missing mechanisms: missing completely at random (MCAR), missing at random (MAR), and not missing at random (NMAR). [@little2014statistical]
\hfill\break
...
_Pay plan_, _Grade_, _Step Rate_, and _Salary_ are variables with missing data due to time constraint. As mentioned before, an employee's salary information is determined by certain grade, step rate and pay plan. Since all four variables are related with each other, the percentage of missing data for them is generally the same overtime. Specifically, the proportion of missingness increases as year decreases. (Figure \@ref(fig:nonfixedmissing)) This corresponds to the fact that the earliest year recorded in the OPM data is 1988, but employees starting before 1988 are also included.
\hfill\break
